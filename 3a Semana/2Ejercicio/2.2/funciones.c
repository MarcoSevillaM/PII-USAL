...
matFloatRef multiplicarMatFloat(matFloatRef a, matFloatRef b, intRef errNum)
{
	...
	*errNum = fallaMatrizFloat(a);
	*errNum = fallaMatrizFloat(b);
	if(*errNum != 0) return NULL;
	c = crearMatFloat(a->numFil, b->numCol, errNum);
...
matIntRef multiplicarMatInt(matIntRef a, matIntRef b, intRef errNum)
{
	...
	matIntRef c = NULL;
	if (a==NULL)
		printf("La primera matriz null");
	*errNum = fallaMatrizInt(a);
	*errNum = fallaMatrizInt(b);
	if(*errNum != 0) return NULL;
	c = crearMatInt(a->numFil, b->numCol, errNum);
	if(a->numCol != b->numFil){
		*errNum = -5;
		return NULL;
	}
